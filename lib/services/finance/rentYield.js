...
/**
 * `rentDataMatchFor`'s `coverage` values, most to least specific:
 * - `override`: the listing has its own `cold_rent_override`, set by hand on the detail page.
 * - `area`: a district/neighborhood within a matched city has its own Mietspiegel-sourced figure.
 * - `city`: the city matched, using its flat city-wide figure.
 * - `default`: no city matched at all - this is the single nationwide `defaultRentPerSqm` fallback,
 *   which says nothing about the listing's actual location.
 * - `none`: no estimate could be produced at all (no match and no configured default).
 * @typedef {'override'|'area'|'city'|'default'|'none'} RentDataCoverage
 */

/**
 * Resolve the cold rent per m²/month for a listing, and report how confidently - matching known
 * city names against its address and title text, falling back to the data file's
 * `defaultRentPerSqm` when no configured city name appears in either field.
 *
 * The `coverage` this returns is what lets a caller distinguish a real, sourced local figure from
 * the single nationwide `defaultRentPerSqm` guess: rent-data.json only has hand-curated entries
 * for the handful of cities Fredy's jobs actually target (see the file's own `_comment` for how
 * each one was sourced), so most listings nationwide fall through to `default` today - a yield
 * built on that isn't wrong exactly, but it isn't a *place-specific* number either, and a rating
 * pass that doesn't know the difference will trust it more than it should.
 *
 * `cold_rent_override` wins over all of that when it is set. The city match is a substring test
 * against scraped text, and two portals can describe the same real flat with address strings that
 * hit two different cities/districts - rent-data.json isn't wrong in that case, the scraped text
 * just sent the lookup down the wrong branch. A human who has actually looked at the listing can
 * say what the cold rent really is; nothing here should second-guess them.
 *
 ...
 * city's listings.
 *
 * @param {{ address?: string, title?: string, size?: number, cold_rent_override?: number|null }} listing
 * @returns {{ rentPerSqm: number|null, coverage: RentDataCoverage }}
 */
export function rentDataMatchFor(listing) {
  if (listing.cold_rent_override != null && listing.size) {
    return { rentPerSqm: listing.cold_rent_override / listing.size, coverage: 'override' };
  }
  ...
  for (const [city, cityEntry] of Object.entries(cities)) {
    ...
    for (const [area, rentPerSqm] of Object.entries(cityEntry.areas)) {
      if (haystack.includes(area)) {
        return { rentPerSqm, coverage: 'area' };
      }
    }
    return { rentPerSqm: cityEntry.default, coverage: cityEntry.default == null ? 'none' : 'city' };
  }
  return { rentPerSqm: defaultRentPerSqm, coverage: defaultRentPerSqm == null ? 'none' : 'default' };
}

/**
 * Just the number from {@link rentDataMatchFor}, for the (majority of) callers that only need the
 * rent estimate itself and not how confidently it was derived.
 *
 * @param {{ address?: string, title?: string, size?: number, cold_rent_override?: number|null }} listing
...
export function rentPerSqmFor(listing) {
  return rentDataMatchFor(listing).rentPerSqm;
}
