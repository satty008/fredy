 ...
 */
const LOAN_RATE_PERCENT = 4.2; // 100% financing at this rate is the handoff's worst-case screening assumption
 ...
 * 100% financing at 4.2%/2%, a married/€100k-taxable-income buyer profile, and immocockpit's own
 ...
 * @returns {{ verdict: 'good'|'maybe'|'bad', score: number, netYieldPercent: number,
 *   cfAfterTaxMonthly: number, equityReturnPercent: number, dscr: number }|null}
 */
export function immocockpitVerdictFor(listing) {
  ...
  // 100% financing: the loan covers the purchase price itself, so the only cash actually needed
  // up front is the purchase costs - which is also, therefore, the equity figure below.
  const equity = totalInvestment - price;
  const interestMonthly = ((LOAN_RATE_PERCENT / 100) * price) / 12;
  const principalMonthly = ((LOAN_AMORTIZATION_PERCENT / 100) * price) / 12;
