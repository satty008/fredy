...
import { grossYieldPercent, netYieldPercent } from '../finance/rentYield.js';
import { immocockpitVerdictFor } from '../finance/immocockpitVerdict.js';
...
/**
 * Builds a `column IN (@p0, @p1, ...)` SQL fragment for a list of values, writing each into
 * `params` under a fresh, prefix-scoped key so several IN-clauses in the same query never
 * collide on parameter names.
 *
 * @param {string} column - Fully-qualified column, e.g. `l.job_id`.
 * @param {Array} values - Raw candidate values; blanks and nullish entries are dropped.
 * @param {string} paramPrefix - Unique per call site.
 * @param {Object} params - Mutated in place with the new bind parameters.
 * @returns {string|null} The fragment, or null when nothing survived filtering - SQLite rejects
 *   `IN ()`, so an empty list must not become a clause at all rather than one that matches nothing.
 */
function inClause(column, values, paramPrefix, params) {
  const list = (Array.isArray(values) ? values : [values]).filter((v) => v != null && String(v).trim().length > 0);
  if (list.length === 0) {
    return null;
  }
...
const parseListingStatus = (row) => {
  ...
  // Computed at read time rather than stored, so it stays correct if rent-data.json is edited.
  const yieldPercent = grossYieldPercent(row);
  row.grossYieldPercent = yieldPercent == null ? null : Math.round(yieldPercent * 10) / 10;
  const netYield = netYieldPercent(row);
  row.netYieldPercent = netYield == null ? null : Math.round(netYield * 10) / 10;
  // Same read-time reasoning as the yield fields above, and the reason immocockpitVerdictFilter
  // below can't be pushed down to SQL: the verdict depends on price+size+city jointly through a
  // formula that also reads rent-data.json/standort-data.json, not on a single stored column.
  const immocockpitAnalysis = immocockpitVerdictFor(row);
  row.immocockpitVerdict = immocockpitAnalysis?.verdict ?? null;
  row.immocockpitAnalysis = immocockpitAnalysis;
  return row;
};
 ...
 *
 * @param {Object} [params]
 * @param {string|string[]|null} [params.jobId] - One job id, or several now that the job filter
 *   is multi-select.
 * @param {string|string[]|null} [params.jobName]
...
export const getAvailableProviders = ({
  ...
  const jobIdClause = inClause('l.job_id', jobId, 'jobId', params);
  if (jobIdClause) {
    whereParts.push(`(${jobIdClause})`);
  } else {
    const jobNameClause = inClause('j.name', jobName, 'jobName', params);
    if (jobNameClause) {
      whereParts.push(`(${jobNameClause})`);
    }
  }
  if (hiddenOnly) {
 ...
 * @param {string|string[]} [params.jobNameFilter] - One job name, or several.
 * @param {string|string[]} [params.jobIdFilter] - One job id, or several - preferred over
 *   jobNameFilter when both are given.
 * @param {string|string[]} [params.providerFilter] - One provider name, or several.
 ...
 * @param {('good'|'maybe'|'bad'|'none')|('good'|'maybe'|'bad'|'none')[]} [params.aiVerdictFilter] -
 *   One AI verdict, or several (OR'ed together).
 * @param {('good'|'maybe'|'bad'|'none')|('good'|'maybe'|'bad'|'none')[]} [params.immocockpitVerdictFilter] -
 *   One or several of the computed-at-read-time immocockpit verdicts (see parseListingStatus).
 *   'none' matches listings it can't be computed for. Unlike every other filter here, this one
 *   can't be expressed in the WHERE clause, so when set the whole matching set is scored and
 *   paginated in JS instead of SQL.
 * @param {('good'|'maybe'|'bad'|'none')|('good'|'maybe'|'bad'|'none')[]} [params.priceFactorFilter] -
 *   Same idea as immocockpitVerdictFilter, but keyed to immocockpitAnalysis.priceFactorTier (the
 *   Kaufpreisfaktor's own good/maybe/bad banding) rather than the overall verdict - a listing can
 *   fail the IC verdict on cashflow/DSCR while its price factor alone is fine, or the other way
 *   round, so the two need to be filterable independently.
...
export const queryListings = ({
  ...
  // Prefer filtering by job id(s) when provided (unambiguous and robust); one or several, OR'ed.
  const jobIdClause = inClause('l.job_id', jobIdFilter, 'jobId', params);
  if (jobIdClause) {
    whereParts.push(`(${jobIdClause})`);
  } else {
    // Fallback to exact job name match
    const jobNameClause = inClause('j.name', jobNameFilter, 'jobName', params);
    if (jobNameClause) {
      whereParts.push(`(${jobNameClause})`);
    }
  }
  // providerFilter: one provider name, or several now that the filter is multi-select.
  const providerClause = inClause('l.provider', providerFilter, 'providerName', params);
  if (providerClause) {
    whereParts.push(`(${providerClause})`);
  }
  ...
  // aiVerdictFilter: one or several of 'good'|'maybe'|'bad' -> IN on the structured verdict
  // column; 'none' -> listings an AI pass hasn't rated yet. The two arms OR together, so
  // ['good', 'none'] reads as "good, or not yet rated".
  {
    const wanted = (Array.isArray(aiVerdictFilter) ? aiVerdictFilter : [aiVerdictFilter])
      .filter((v) => typeof v === 'string')
      .map((v) => v.toLowerCase());
    const arms = [];
    const valueClause = inClause(
      'l.ai_verdict',
      wanted.filter((v) => ['good', 'maybe', 'bad'].includes(v)),
      'aiVerdictValue',
      params,
    );
    if (valueClause) {
      arms.push(valueClause);
    }
    if (wanted.includes('none')) {
      arms.push('l.ai_verdict IS NULL');
    }
    if (arms.length > 0) {
  ...
  // immocockpitVerdictFilter can't join the WHERE clause above: unlike the affordability band,
  // which is reducible to a price range because its verdict is monotonic in one stored column,
  // the immocockpit verdict depends on price+size+city jointly through a formula that also reads
  // rent-data.json/standort-data.json (see parseListingStatus above) - there is no SQL predicate
  // over stored columns equivalent to it. So when this filter is active, every row matching the
  // other filters is fetched unpaginated, scored, and filtered in JS first; only then is the
  // requested page sliced off, since the true totalNumber can't be known before scoring either.
  const wantedIcVerdicts = (
    Array.isArray(immocockpitVerdictFilter) ? immocockpitVerdictFilter : [immocockpitVerdictFilter]
  )
    .filter((v) => typeof v === 'string' && ['good', 'maybe', 'bad', 'none'].includes(v.toLowerCase()))
    .map((v) => v.toLowerCase());

  const wantedPriceFactorTiers = (Array.isArray(priceFactorFilter) ? priceFactorFilter : [priceFactorFilter])
    .filter((v) => typeof v === 'string' && ['good', 'maybe', 'bad', 'none'].includes(v.toLowerCase()))
    .map((v) => v.toLowerCase());

  if (wantedIcVerdicts.length > 0 || wantedPriceFactorTiers.length > 0) {
    ...
    const wantedIcVerdictSet = new Set(wantedIcVerdicts);
    const wantedPriceFactorTierSet = new Set(wantedPriceFactorTiers);
    const filtered = allRows.filter((row) => {
      if (
        wantedIcVerdictSet.size > 0 &&
        !wantedIcVerdictSet.has(row.immocockpitVerdict == null ? 'none' : row.immocockpitVerdict)
      ) {
        return false;
      }
      if (
        wantedPriceFactorTierSet.size > 0 &&
        !wantedPriceFactorTierSet.has(row.immocockpitAnalysis?.priceFactorTier ?? 'none')
      ) {
        return false;
      }
      return true;
    ...
    return {
      ...
      page: safePage,
      result: attachTravelTimes(filtered.slice(offset, offset + safePageSize)),
    };
  }
...
/**
 * Set or clear the structured AI review verdict for a single listing.
 *
 * Kept separate from the free-text notes column so the UI/API can filter and sort on it
 * without parsing text - `notes` still carries the human-readable reasoning.
 *
 * @param {string} id - The listing ID.
 * @param {('good'|'maybe'|'bad'|null)} verdict - New verdict, or null to clear.
 * @returns {number} Number of rows affected (0 if listing not found).
 */
export const setListingAiVerdict = (id, verdict) => {
  if (!id) return 0;
  ...
  if (normalized != null && !allowed.includes(normalized)) {
    throw new Error(`Invalid AI verdict: ${verdict}`);
  }
  const res = SqliteConnection.execute(`UPDATE listings SET ai_verdict = @verdict WHERE id = @id`, {
    id,
    verdict: normalized,
  });
  return res?.changes ?? 0;
...
/**
 * Set or clear the cold-rent override `rentPerSqmFor` prefers over its own rent-data.json lookup.
 *
 * A monthly EUR figure rather than a per-m² rate, because that's the number shown on the finance
 * card and the one a person comparing against comps actually has in hand - `rentPerSqmFor` divides
 * by the listing's size itself. `null` clears the override and returns the listing to whatever
 * rent-data.json says, e.g. once the underlying address text gets fixed instead.
 *
 * @param {string} id - The listing ID.
 * @param {number|null} coldRent - EUR/month, or null to clear the override.
 * @returns {number} Number of rows affected (0 if the listing does not exist).
 */
export const setColdRentOverride = (id, coldRent) => {
  if (!id) return 0;
  const value = coldRent == null ? null : Number(coldRent);
  if (value != null && (!Number.isFinite(value) || value < 0)) {
    throw new Error(`Invalid cold rent override: ${coldRent}`);
  }
  const res = SqliteConnection.execute(`UPDATE listings SET cold_rent_override = @value WHERE id = @id`, {
    id,
    value,
  });
  return res?.changes ?? 0;
