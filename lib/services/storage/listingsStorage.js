 ...
 * @param {('good'|'maybe'|'bad'|'none')} [params.immocockpitVerdictFilter] - Filter by the
 *   computed-at-read-time immocockpit verdict (see parseListingStatus). 'none' matches listings
 *   it can't be computed for. Unlike every other filter here, this one can't be expressed in the
 *   WHERE clause, so when set the whole matching set is scored and paginated in JS instead of SQL.
...
export const queryListings = ({
  ...
  // aiVerdictFilter: 'good'|'maybe'|'bad' -> equality on the structured verdict column;
  // 'none' -> listings an AI pass hasn't rated yet.
  if (aiVerdictFilter === 'none') {
    whereParts.push('(l.ai_verdict IS NULL)');
  } else if (typeof aiVerdictFilter === 'string' && ['good', 'maybe', 'bad'].includes(aiVerdictFilter.toLowerCase())) {
    params.aiVerdictValue = aiVerdictFilter.toLowerCase();
    whereParts.push('(l.ai_verdict = @aiVerdictValue)');
  }
  ...
  const wantedIcVerdict =
    typeof immocockpitVerdictFilter === 'string' &&
    ['good', 'maybe', 'bad', 'none'].includes(immocockpitVerdictFilter.toLowerCase())
      ? immocockpitVerdictFilter.toLowerCase()
      : null;

  if (wantedIcVerdict != null) {
    ...
    const filtered =
      wantedIcVerdict === 'none'
        ? allRows.filter((row) => row.immocockpitVerdict == null)
        : allRows.filter((row) => row.immocockpitVerdict === wantedIcVerdict);
