...
You will also be given a "Rent data coverage" field. rent-data.json only has hand-curated,
Mietspiegel-sourced figures for a limited set of cities/districts - most listings nationwide fall
through to its single nationwide fallback average instead, which is not specific to this
listing's actual location at all. Coverage values:
- "area match" / "city match": a real, location-specific figure. Trust the yield number(s) at
  face value, per the bands above.
- "nationwide fallback average": both yield fields may still be non-null, but neither is
  location-specific - treat them the way you'd treat a gross-yield-only fallback (wider,
  conservative bands, capped at MAYBE unless the listing's own text states an explicit healthy
  yield figure you can point to directly), and say explicitly in your reasoning that the number
  is a generic nationwide average, not real local data.
- "no rent data": both yield fields are null - identical to the netYieldPercent/grossYieldPercent
  both-null case, nothing further to add here.
