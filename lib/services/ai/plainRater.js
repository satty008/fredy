...
import { rentDataMatchFor } from '../finance/rentYield.js';
...
/**
 * Human-readable label for a {@link rentDataMatchFor} coverage value, matched to the wording
 * DEFAULT_RATING_INSTRUCTIONS tells the model to look for and act on.
 * @type {Record<import('../finance/rentYield.js').RentDataCoverage, string>}
 */
const COVERAGE_LABELS = {
  override: 'manual override (cold rent set by hand for this listing)',
  area: 'area match (real local data)',
  city: 'city match (real local data)',
  default: 'nationwide fallback average (not specific to this location)',
  none: 'no rent data',
};
...
function formatListingForPrompt(listing) {
  const { coverage } = rentDataMatchFor(listing);
  const lines = [
    ...
    `Rent data coverage: ${COVERAGE_LABELS[coverage]}`,
  ];
  return lines.join('\n');
}
