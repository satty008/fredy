...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    // icVerdictFilter/priceFactorFilter are computed at read time in JS (see queryListings), so
    // normalizeListingFilters below deliberately leaves them out - they can't be pushed into the
    // WHERE clause it builds, which bulk delete also runs. Same allowlist as the AI verdict
    // shared helper covers; 'none' means immocockpitVerdictFor couldn't compute one (not a 'buy'
    // job, or no rent estimate/Bundesland for the listing's city).
