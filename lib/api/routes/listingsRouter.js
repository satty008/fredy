...
import { immocockpitVerdictFor } from '../../services/finance/immocockpitVerdict.js';
...
export default async function listingsPlugin(fastify) {
  fastify.get('/table', async (request) => {
    ...
    const normalizedAiVerdict =
      typeof aiVerdictFilter === 'string' && allowedAiVerdicts.includes(aiVerdictFilter.toLowerCase())
        ? aiVerdictFilter.toLowerCase()
        : undefined;
    ...
    const withVerdict = (page) => ({
      ...
      result: page.result.map((listing) => {
        // Doesn't depend on a finance profile, so it's attached unconditionally - it's the same
        // verdict immocockpit's own "Fredy handoff" screening default would compute, so the badge
        // on a row can never disagree with what opening the listing in immocockpit would show.
        // immocockpitAnalysis carries the underlying numbers too (same shape as the detail
        // endpoint) so callers like the AI rater can cite them without a separate detail fetch.
        const immocockpitAnalysis = immocockpitVerdictFor(listing);
        return {
          ...listing,
          affordabilityVerdict:
            thresholds == null ? null : verdictForListing(listing.price, listing.dealType, thresholds),
          immocockpitVerdict: immocockpitAnalysis?.verdict ?? null,
          immocockpitAnalysis,
          // Location context (Prognos Zukunftsatlas rank, area price/rent level) from
          // immocation's Standort Tool, when the listing's city has an entry - see
          // standortData.js. Attached here too so the AI rater can read it straight off the
          // /table response it already fetches, without a per-listing detail call.
  ...
  fastify.get('/:listingId', async (request, reply) => {
    ...
    return {
      ...
      // Same enum shape as the overview rows' badge; immocockpitAnalysis below carries the
      // underlying numbers for the detail page's richer tooltip.
      immocockpitVerdict: immocockpitAnalysis?.verdict ?? null,
      immocockpitAnalysis,
