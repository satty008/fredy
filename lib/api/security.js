...
/**
 * Authenticates a request whose session is missing/expired against its MCP bearer token,
 * so headless/scripted clients can reuse a user's existing MCP token instead of holding a
 * session cookie. Populates `request.session.currentUser` on success so downstream handlers
 * work exactly as they do for cookie-authenticated requests.
 * @param {import('fastify').FastifyRequest} request
 * @returns {boolean} True if the request was authenticated via bearer token.
 */
function tryBearerAuth(request) {
  if (!request.headers) return false;
  const auth = authenticateRequest(request);
  if (!auth?.userId) return false;
  const user = userStorage.getUser(auth.userId);
 ...
 *
 * Falls back to MCP bearer token auth (`Authorization: Bearer fredy_<token>`) when there is
 * no valid session, so scripted clients can authenticate the same way the MCP endpoint does.
 * @param {import('fastify').FastifyRequest} request
 * @param {import('fastify').FastifyReply} reply
 */
export async function authHook(request, reply) {
  if (await isUnauthorized(request)) {
    if (tryBearerAuth(request)) return;
