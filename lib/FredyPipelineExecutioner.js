...
import { grossYieldPercent } from './services/finance/rentYield.js';
...
class FredyPipelineExecutioner {
  ...
  constructor(providerConfig, job, providerId, similarityCache, browser, options = {}) {
    ...
    /** @type {'rent' | 'buy' | undefined} */
    this._jobDealType = job.dealType;
    /** @type {string} */
  ...
  /**
   * Filter listings based on their specifications (minRooms, minSize, maxPrice, minRoiPercent).
   *
   ...
   *
   * minRoiPercent only applies to 'buy' jobs - a rental's asking price isn't a purchase price,
   * so a gross-yield estimate against it would be meaningless. A listing whose yield can't be
   * estimated (no rent data for its city) is kept rather than dropped: an unknown yield means
   * "cannot judge", not "fails the filter".
   *
  ...
  _filterBySpecs(newListings) {
    const { minRooms, minSize, maxPrice, minRoiPercent } = this._jobSpecFilter || {};

    // If no specs are set, return all listings
    if (!minRooms && !minSize && !maxPrice && !minRoiPercent) {
      return newListings;
    }

    const checkRoi = minRoiPercent && this._jobDealType === 'buy';
    const keptListings = newListings.filter((listing) => {
      let filterOut =
      ...
      if (!filterOut && checkRoi) {
        const yieldPercent = grossYieldPercent(listing);
        filterOut = yieldPercent != null && yieldPercent < minRoiPercent;
      }
